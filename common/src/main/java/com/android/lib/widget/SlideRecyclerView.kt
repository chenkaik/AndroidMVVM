...
class SlideRecyclerView @JvmOverloads constructor(
    ...
    private var mLastX = 0f // 滑动过程中记录上次触碰点X = 0f
    private var mFirstX = 0f
    private var mFirstY = 0f // 首次触碰范围 = 0f
    private var mIsSlide = false // 是否滑动子View = false
    private var mFlingView: ViewGroup? = null // 触碰的子View

    private var mPosition = 0 // 触碰的view的位置 = 0
    private var mMenuViewWidth = 0 // 菜单按钮宽度 = 0
...
// 2.如果x方向的侧滑距离大于y方向滑动距离，且x方向达到最小滑动距离；
    ...
    override fun onTouchEvent(e: MotionEvent): Boolean {
        if (mIsSlide && mPosition != INVALID_POSITION) {
            ...
            return true
        } else { // 此处防止RecyclerView正常滑动时，还有菜单未关闭
            closeMenu()
